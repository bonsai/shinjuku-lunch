...
export default function RestaurantCard({ r }: { r: Restaurant }) {
  return (
    <Link
      ...
      className="block rounded-xl border border-zinc-700 bg-zinc-800/50 p-4 hover:bg-zinc-700/50 transition-colors"
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {r.latitude && r.longitude && (
          <a
            ...
            className="shrink-0 rounded-lg bg-zinc-700 px-2 py-1 text-xs text-blue-400 hover:bg-zinc-600"
          >
            Map
          </a>
        )}
      ...
      {r.notes && (
        <p className="mt-2 text-sm text-zinc-400 line-clamp-2">{r.notes}</p>
      )}
    </Link>
  )
}
